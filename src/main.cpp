...
int main() {
	...
	while (window.isOpen()) {
		...
		glUniform2f(glGetUniformLocation(programID, "scaleSpeed"), 0.5f * cos(elapsedTime*scaleSpeed), 0.5f * sin(elapsedTime*scaleSpeed));
		...
		//glUniform2f(glGetUniformLocation(programID, "screenSize"), 400, 400);

		glUniform2f(glGetUniformLocation(programID, "mouse"), mouseX, mouseY);

		rotationAngle += 0.001f;

		glUniform1f(glGetUniformLocation(programID, "rotationAngle"),  rotationAngle);


		glUniform1f(glGetUniformLocation(programID, "rows"), 4);
		glUniform1f(glGetUniformLocation(programID, "cols"), 4);
